...
bazel_cmd=(
  ...
  "/Users/sangnguyen/Library/Caches/bazelisk/downloads/sha256/a89f446641ab1cce603691cb7030865d1fb014e260ee5710615516e3cacd2414/bin/bazel"
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj++internal+rules_xcodeproj_generated/generator/platform/ios/xcodeproj/xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj++internal+rules_xcodeproj_generated/generator/platform/ios/xcodeproj/xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(
    comm -23 \
      <(printf '%s\n' "${target_ids[@]}" | sort) \
      <(sort "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj++internal+rules_xcodeproj_generated/generator/platform/ios/xcodeproj/xcodeproj_target_ids")
  )
