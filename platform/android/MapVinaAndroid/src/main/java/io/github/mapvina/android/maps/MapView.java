...
import io.github.mapvina.android.exceptions.MapVinaConfigurationException;
 ...
 * </p>
 * <p>
 * Use of {@code MapView} requires a MapVina API access token.
 * Obtain an access token on the <a href="https://www.mapbox.com/studio/account/tokens/">MapVina account page</a>.
 * </p>
...
public class MapView extends FrameLayout implements NativeMapView.ViewCallback {
  ...
  MapVinaMapOptions mapvinaMapOptions;
  ...
  public MapView(@NonNull Context context) {
    ...
    initialize(context, MapVinaMapOptions.createFromAttributes(context));
  }
  ...
  public MapView(@NonNull Context context, @Nullable AttributeSet attrs) {
    ...
    initialize(context, MapVinaMapOptions.createFromAttributes(context, attrs));
  }
  ...
  @UiThread
  public MapView(@NonNull Context context, @Nullable MapVinaMapOptions options) {
    super(context);
    Timber.d("MapView constructed with context and MapVinaMapOptions");
    initialize(context, options == null ? MapVinaMapOptions.createFromAttributes(context) : options);
  }

  @CallSuper
  @UiThread
  protected void initialize(@NonNull final Context context, @NonNull final MapVinaMapOptions options) {
    if (isInEditMode()) {
    ...
    if (!MapVina.hasInstance()) {
      throw new MapVinaConfigurationException();
    }
    ...
    setContentDescription(context.getString(R.string.mapvina_mapActionDescription));
  ...
  private void initializeMap() {
    ...
    // setup components for MapVinaMap creation
    ...
    // MapVinaMap
    List<MapVinaMap.OnDeveloperAnimationListener> developerAnimationListeners = new ArrayList<>();
    mapvinaMap = new MapVinaMap(nativeMapView, transform, uiSettings, proj, registerTouchListener, cameraDispatcher,
            developerAnimationListeners);
    mapvinaMap.injectAnnotationManager(annotationManager);
    ...
    // LocationComponent
    mapvinaMap.injectLocationComponent(new LocationComponent(mapvinaMap, transform, developerAnimationListeners));
    ...
    nativeMapView.setReachability(MapVina.isConnected());

    // initialise MapVinaMap
    if (savedInstanceState == null) {
      mapvinaMap.initialise(context, mapvinaMapOptions);
    } else {
      mapvinaMap.onRestoreInstanceState(savedInstanceState);
    }
  ...
  protected CompassView initialiseCompassView() {
    ...
    compassView.setContentDescription(getResources().getString(R.string.mapvina_compassContentDescription));
  ...
  protected ImageView initialiseAttributionView() {
    ...
    attrView.setContentDescription(getResources().getString(R.string.mapvina_attributionsIconContentDescription));
    attrView.setImageDrawable(BitmapUtils.getDrawableFromRes(getContext(), R.drawable.mapvina_info_bg_selector));
    // inject widgets with MapVinaMap
    attrView.setOnClickListener(attributionClickListener = new AttributionClickListener(getContext(), mapvinaMap));
    return attrView;
  }
  ...
  protected ImageView initialiseLogoView() {
    ...
    logoView.setImageDrawable(BitmapUtils.getDrawableFromRes(getContext(), R.drawable.mapvina_logo_icon));
    return logoView;
  }
  ...
  private MapVinaMap.OnCompassAnimationListener createCompassAnimationListener(@NonNull final CameraChangeDispatcher
                                                                                      cameraChangeDispatcher) {
    return new MapVinaMap.OnCompassAnimationListener() {
      @Override
  ...
  private OnClickListener createCompassClickListener(@NonNull final CameraChangeDispatcher cameraChangeDispatcher) {
    return new OnClickListener() {
      ...
      public void onClick(View v) {
        if (mapvinaMap != null && compassView != null) {
          if (focalPoint != null) {
            mapvinaMap.setFocalBearing(0, focalPoint.x, focalPoint.y, TIME_MAP_NORTH_ANIMATION);
          } else {
            mapvinaMap.setFocalBearing(
              0, mapvinaMap.getWidth() / 2, mapvinaMap.getHeight() / 2,
              TIME_MAP_NORTH_ANIMATION);
          }
          cameraChangeDispatcher.onCameraMoveStarted(MapVinaMap.OnCameraMoveStartedListener.REASON_API_ANIMATION);
   ...
   * {@link MapVina#getInstance(Context, String, WellKnownTileServer)}
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    if (savedInstanceState != null && savedInstanceState.getBoolean(MapVinaConstants.STATE_HAS_SAVED_STATE)) {
      this.savedInstanceState = savedInstanceState;
    }
  }

  private void initializeDrawingSurface(MapVinaMapOptions options) {
  ...
  public void onSaveInstanceState(@NonNull Bundle outState) {
    if (mapvinaMap != null) {
      outState.putBoolean(MapVinaConstants.STATE_HAS_SAVED_STATE, true);
      mapvinaMap.onSaveInstanceState(outState);
    }
  }
  ...
  public void onLowMemory() {
    if (nativeMapView != null && mapvinaMap != null && !destroyed) {
   ...
   * Can be overwritten using {@link MapVinaMapOptions#pixelRatio(float)}.
   *
   * @return the current map pixel ratio
   */
  public float getPixelRatio() {
    // check is user defined his own pixel ratio value
    float pixelRatio = mapvinaMapOptions.getPixelRatio();
    if (pixelRatio == 0) {
   ...
   * <p>
   * Calling {@link MapVinaMap#snapshot(MapVinaMap.SnapshotReadyCallback)} from this callback
  ...
  /**
   * Sets a callback object which will be triggered when the {@link MapVinaMap} instance is ready to be used.
   *
  ...
  public void getMapAsync(final @NonNull OnMapReadyCallback callback) {
    if (mapvinaMap == null) {
      ...
      callback.onMapReady(mapvinaMap);
    }
  }
  ...
  @Nullable
  MapVinaMap getMapVinaMap() {
    return mapvinaMap;
  }

  void setMapVinaMap(MapVinaMap mapvinaMap) {
  ...
  private class InitialRenderCallback implements OnDidFinishRenderingFrameWithStatsListener {
    ...
    public void onDidFinishRenderingFrame(boolean fully, RenderingStats stats) {
      if (mapvinaMap != null && mapvinaMap.getStyle() != null && mapvinaMap.getStyle().isFullyLoaded()) {
        renderCount++;
        if (renderCount == 3) {
  ...
  private class GesturesManagerInteractionListener implements MapVinaMap.OnGesturesManagerInteractionListener {

    @Override
    public void onAddMapClickListener(MapVinaMap.OnMapClickListener listener) {
      mapGestureDetector.addOnMapClickListener(listener);
    }

    @Override
    public void onRemoveMapClickListener(MapVinaMap.OnMapClickListener listener) {
      mapGestureDetector.removeOnMapClickListener(listener);
    }

    @Override
    public void onAddMapLongClickListener(MapVinaMap.OnMapLongClickListener listener) {
      mapGestureDetector.addOnMapLongClickListener(listener);
    }

    @Override
    public void onRemoveMapLongClickListener(MapVinaMap.OnMapLongClickListener listener) {
      mapGestureDetector.removeOnMapLongClickListener(listener);
    }

    @Override
    public void onAddFlingListener(MapVinaMap.OnFlingListener listener) {
      mapGestureDetector.addOnFlingListener(listener);
    }

    @Override
    public void onRemoveFlingListener(MapVinaMap.OnFlingListener listener) {
      mapGestureDetector.removeOnFlingListener(listener);
    }

    @Override
    public void onAddMoveListener(MapVinaMap.OnMoveListener listener) {
      mapGestureDetector.addOnMoveListener(listener);
    }

    @Override
    public void onRemoveMoveListener(MapVinaMap.OnMoveListener listener) {
      mapGestureDetector.removeOnMoveListener(listener);
    }

    @Override
    public void onAddRotateListener(MapVinaMap.OnRotateListener listener) {
      mapGestureDetector.addOnRotateListener(listener);
    }

    @Override
    public void onRemoveRotateListener(MapVinaMap.OnRotateListener listener) {
      mapGestureDetector.removeOnRotateListener(listener);
    }

    @Override
    public void onAddScaleListener(MapVinaMap.OnScaleListener listener) {
      mapGestureDetector.addOnScaleListener(listener);
    }

    @Override
    public void onRemoveScaleListener(MapVinaMap.OnScaleListener listener) {
      mapGestureDetector.removeOnScaleListener(listener);
    }

    @Override
    public void onAddShoveListener(MapVinaMap.OnShoveListener listener) {
      mapGestureDetector.addShoveListener(listener);
    }

    @Override
    public void onRemoveShoveListener(MapVinaMap.OnShoveListener listener) {
  ...
  private class MapCallback implements OnDidFinishLoadingStyleListener,
    ...
    void initialised() {
      mapvinaMap.onPreMapReady();
      onMapReady();
      mapvinaMap.onPostMapReady();
    }
    ...
    public void onDidFinishRenderingFrame(boolean fully, RenderingStats stats) {
      if (mapvinaMap != null) {
        mapvinaMap.onUpdateFullyRendered();
      }
    }
    ...
    public void onDidFinishLoadingMap() {
      if (mapvinaMap != null) {
        mapvinaMap.onUpdateRegionChange();
      }
    }
  ...
  private static class AttributionClickListener implements OnClickListener {
    ...
    private AttributionClickListener(@NonNull Context context, @NonNull MapVinaMap mapvinaMap) {
      this.defaultDialogManager = new AttributionDialogManager(context, mapvinaMap);
      this.uiSettings = mapvinaMap.getUiSettings();
    }
