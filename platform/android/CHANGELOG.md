...
They are available with a `-debug` postfix on Maven Central, for example `io.github.mapvina:android-sdk-vulkan-debug`.
...
- `io.github.mapvina:android-sdk` (still OpenGL ES for now, might default to another rendering backend or might choose depending on device support in a future major release).
